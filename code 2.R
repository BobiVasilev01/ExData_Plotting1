...
# Create the line plot with days on x-axis and global active power on y-axis
plot <- ggplot(filtered_data, aes(x = Time, y = Global_active_power)) +
  geom_line(color = "black") +  # Black line for the plot
  labs(title = "Global Active Power over Time", 
       x = "Date", 
       y = "Global Active Power (kilowatts)") +
  scale_x_datetime(date_labels = "%a", date_breaks = "1 day") +  # Correct function name here
  theme_minimal(base_size = 15) +  # Increase font size for clarity
  theme(
...
# Save the plot as plot2.png with 480x480 pixels
ggsave("plot2.png", plot = plot, width = 480/72, height = 480/72, dpi = 72)  # 480x480 in pixels with 72 dpi
